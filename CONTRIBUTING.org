...
*Note*: If there is no special configuration for a given package, there is no
      need to contribute a layer, instead users can either add it to their =dotspacemacs/additional-packages= list or
      make a private layer.
