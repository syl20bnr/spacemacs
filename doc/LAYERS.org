...
- =packages.el= :: the packages configuration
...
** config.el
This file configure the layer, it must at least include a variable called
=<layer>-packages=, which should be a list of all the packages that this layer
needs. It also contains what we call =layer variables= to further configure the
layer. Some valid package specifications are as follows:
...
** packages.el
It contains the actual configuration for the packages included in the layer.
