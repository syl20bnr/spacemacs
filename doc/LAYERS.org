...
1. Spacemacs goes through all the enabled layers and evaluates their files.
   First =packages.el= and =funcs.el= are loaded, but nothing happens from them
   since these files only define functions and variables, then the changes
   introduced by =config.el= are applied.
