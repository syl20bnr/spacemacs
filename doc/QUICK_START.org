...
When you install Spacemacs, what you are really installing is an `emacs.d`
directory, configured so that Emacs runs as Spacemacs. To run Spacemacs,
just run `emacs` as you normally would.
