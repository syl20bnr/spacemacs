...
If a package is not available on MELPA, you must use an local package or a
package recipe. For more details see [[file:LAYERS.org#anatomy-of-a-layer][anatomy of a layer]].

Make sure you [[#activating-a-layer][add]] your layer to your =.dotspacemacs= file and restart to
activate it.

A detailed overview of the loading process and how layers work can be found in
