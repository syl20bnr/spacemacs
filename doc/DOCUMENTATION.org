...
** Installing Packages
All necessary ELPA packages are installed automatically so that you never
have to invoke =package-install= command manually.  This is the case for
both [[Within a layer][packages associated with layers]] as well as
packages listed in =dotspacemacs-additional-packages=.

In fact if you install packages via =package-install= apart from the two
sanctioned methods above, then these are considered /orphaned/ packages. Such
packages are deleted (i.e., uninstalled) automatically the next time emacs
starts up if =dotspacemacs-delete-orphan-packages= is =t= which is the default
value.  You can set this to =nil= to prevent emacs from deleting orphaned
packages on startup.
