...
If any of these core pillars are violated, open an issue and we'll fix it.
...
[[https://github.com/justbur/emacs-which-key][which-key]] is enabled by default, displaying all the available key bindings
...
thing at point=. No matter what the major mode is, it is the same binding to
perform this action.
...
** Crowd-Configured
By defining a very light and easy to understand structure called =configuration layer=, =Spacemacs= makes it easy to contribute additional support.
...
Spacemacs will automatically check for a new version at every startup. When it
detects that a new version is available, an arrow will appear in the modeline.
Click it to update Spacemacs. You must then restart Emacs after updating.
...
If you want to add some configuration for them, consider creating a layer,
...
=Spacemacs= features a synchronization engine for the ELPA packages. This means
...
This effectively makes =Spacemacs= behave like [[https://github.com/gmarik/Vundle.vim][Vundle]].
