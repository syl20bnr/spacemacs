...
sequence (by default ~fd~) and use the ~ESC~ key explicitly. =evil-escape=
does not support being replayed in macros for now.

**** Vim keyboard macros
Spacemacs supports the VIM interface to create macros using ~q~ to start a macro
and ~@~ in normal state.

To start recording a keyboard macro to a register type:

#+begin_comment
q<letter><commands>q
#+end_comment

To execute the recorded macro type:
...
To stop and execute the recorded macro type twice (once to stop and once to
execute):
...
When starting to record a macro the counter is at 0 (you can also set it to a
specific value) then each time the counter is inserted it is incremented
afterwards by 1.
When recording a macro it is possible to insert the current value by pressing ~SPC K k~.
