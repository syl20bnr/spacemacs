...
Note that =center-buffer-mode= will not be enabled if current frame(Emacs window) aspect ration
is less than specified by =spacemacs-space-doc-center-buffer-mode-min-aspect-ratio= variable.
