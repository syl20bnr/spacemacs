;;; space-macs-common.el --- Color theme with a dark and light versions.

;; Copyright (C) 2015-2018 Nasser Alshammari

;; Author: Nasser Alshammari
;; URL: <https://github.com/nashamri/space-macs-theme>
;;
;; Version: 0.1
;; Keywords: color, theme
;; Package-Requires: ((e-macs "24"))

;; Initially created with the help of e-macs-theme-generator, <https://github.com/mswift42/theme-creator>.
...
;; This file is not part of e-macs.

;;; Commentary:

;; This is a color theme for space-macs <https://github.com/syl20bnr/space-macs>.
;; It comes with two versions, dark and light and should work well in
;; a 256 color terminal.

;;; Code:

(defgroup space-macs-theme nil
  "Space-macs-theme options."
  :group 'faces)

(defcustom space-macs-theme-comment-bg t
  "Use a background for comment lines."
  :type 'boolean
  :group 'space-macs-theme)

(defcustom space-macs-theme-comment-italic nil
  "Enable italics for comments and also disable background."
  :type 'boolean
  :group 'space-macs-theme)

(defcustom space-macs-theme-keyword-italic nil
  "Enable italics for keywords."
  :type 'boolean
  :group 'space-macs-theme)

(defcustom space-macs-theme-org-agenda-height nil
...
(defcustom space-macs-theme-org-height t
  "Use varying text heights for org headings."
  :type 'boolean
  :group 'space-macs-theme)

(defcustom space-macs-theme-org-bold t
  "Inherit text bold for org headings"
  :type 'boolean
  :group 'space-macs-theme)

(defcustom space-macs-theme-org-priority-bold t
  "Inherit text bold for priority items in agenda view"
  :type 'boolean
  :group 'space-macs-theme)

(defcustom space-macs-theme-org-highlight nil
  "Highlight org headings."
  :type 'boolean
  :group 'space-macs-theme)

(defcustom space-macs-theme-custom-colors nil
  "Specify a list of custom colors."
  :type 'alist
  :group 'space-macs-theme)

(defcustom space-macs-theme-underline-parens t
...
(defun create-space-macs-theme (variant theme-name)
  (let ((class '((class color) (min-colors 89))) ;;              ~~ Dark ~~                              ~~ Light ~~
    ...
    (cl-loop for (cvar . val) in space-macs-theme-custom-colors
             do (set cvar val))
...
;;;;; basics
     ...
     `(font-lock-comment-face ((,class (:foreground ,(if space-macs-theme-comment-italic comment-light comment) :background ,(when space-macs-theme-comment-bg comment-bg) :slant ,(if space-macs-theme-comment-italic 'italic 'normal)))))
     `(font-lock-constant-face ((,class (:foreground ,const))))
     `(font-lock-doc-face ((,class (:foreground ,meta))))
     `(font-lock-function-name-face ((,class (:foreground ,func :inherit bold))))
     `(font-lock-keyword-face ((,class (:inherit bold :foreground ,keyword :slant ,(if space-macs-theme-keyword-italic 'italic 'normal)))))
...
;;;;; latex
     ...
     `(font-latex-sectioning-0-face ((,class (:inherit bold :foreground ,head3 :height ,(if space-macs-theme-org-height 1.3 1.0) :background ,(when space-macs-theme-org-highlight head3-bg)))))
     `(font-latex-sectioning-1-face ((,class (:inherit bold :foreground ,head4 :height ,(if space-macs-theme-org-height 1.3 1.0) :background ,(when space-macs-theme-org-highlight head4-bg)))))
     `(font-latex-sectioning-2-face ((,class (:inherit bold :foreground ,head1 :height ,(if space-macs-theme-org-height 1.3 1.0) :background ,(when space-macs-theme-org-highlight head1-bg)))))
     `(font-latex-sectioning-3-face ((,class (:inherit bold :foreground ,head2 :height ,(if space-macs-theme-org-height 1.2 1.0) :background ,(when space-macs-theme-org-highlight head2-bg)))))
     `(font-latex-sectioning-4-face ((,class (:bold nil :foreground ,head3 :height ,(if space-macs-theme-org-height 1.1 1.0) :background ,(when space-macs-theme-org-highlight head3-bg)))))
     `(font-latex-sectioning-5-face ((,class (:bold nil :foreground ,head4 :background ,(when space-macs-theme-org-highlight head4-bg)))))
...
;;;;; markdown
     `(markdown-header-face-1 ((,class (:inherit bold :foreground ,head1 :height ,(if space-macs-theme-org-height 1.3 1.0) :background ,(when space-macs-theme-org-highlight head1-bg)))))
     `(markdown-header-face-2 ((,class (:inherit bold :foreground ,head2 :height ,(if space-macs-theme-org-height 1.2 1.0) :background ,(when space-macs-theme-org-highlight head2-bg)))))
     `(markdown-header-face-3 ((,class (:bold nil :foreground ,head3 :height ,(if space-macs-theme-org-height 1.1 1.0) :background ,(when space-macs-theme-org-highlight head3-bg)))))
     `(markdown-header-face-4 ((,class (:bold nil :foreground ,head4 :background ,(when space-macs-theme-org-highlight head4-bg)))))
...
;;;;; org
     ...
     `(org-agenda-date ((,class (:foreground ,var :height ,(if space-macs-theme-org-agenda-height 1.1 1.0)))))
     `(org-agenda-date-today ((,class (:foreground ,keyword :inherit bold :height ,(if space-macs-theme-org-agenda-height 1.3 1.0)))))
     `(org-agenda-date-weekend ((,class (:inherit bold :foreground ,var))))
     `(org-agenda-done ((,class (:foreground ,suc :height ,(if space-macs-theme-org-agenda-height 1.2 1.0)))))
     ...
     `(org-document-title ((,class (:foreground ,func :inherit bold :height ,(if space-macs-theme-org-height 1.4 1.0) :underline t))))
     ...
     `(org-level-1 ((,class (:inherit bold :bold ,(if space-macs-theme-org-bold 'unspecified nil) :foreground ,head1 :height ,(if space-macs-theme-org-height 1.3 1.0) :background ,(when space-macs-theme-org-highlight head1-bg)))))
     `(org-level-2 ((,class (:inherit bold :bold ,(if space-macs-theme-org-bold 'unspecified nil) :foreground ,head2 :height ,(if space-macs-theme-org-height 1.2 1.0) :background ,(when space-macs-theme-org-highlight head2-bg)))))
     `(org-level-3 ((,class (:bold nil :foreground ,head3 :height ,(if space-macs-theme-org-height 1.1 1.0) :background ,(when space-macs-theme-org-highlight head3-bg)))))
     `(org-level-4 ((,class (:bold nil :foreground ,head4 :background ,(when space-macs-theme-org-highlight head4-bg)))))
     ...
     `(org-priority ((,class (:foreground ,war :inherit bold :bold ,(if space-macs-theme-org-priority-bold 'unspecified nil)))))
     `(org-quote ((,class (:inherit org-block :slant italic))))
     `(org-scheduled ((,class (:foreground ,comp))))
     `(org-scheduled-today ((,class (:foreground ,func :height ,(if space-macs-theme-org-agenda-height 1.2 1.0)))))
...
;;;;; show-paren
     `(show-paren-match ((,class (:foreground ,mat :inherit bold  :underline ,(when space-macs-theme-underline-parens t)))))
     `(show-paren-match-expression ((,class (:background ,green-bg-s))))
     `(show-paren-mismatch ((,class (:foreground ,err :inherit bold :underline ,(when space-macs-theme-underline-parens t)))))

;;;;; smartparens
     `(sp-pair-overlay-face ((,class (:background ,highlight :foreground nil))))
     `(sp-show-pair-match-face ((,class (:foreground ,mat :inherit bold  :underline ,(when space-macs-theme-underline-parens t)))))
...
;;;;; space-macs-specific
     `(space-macs-transient-state-title-face ((,class (:background nil :foreground ,comp :box nil :inherit bold))))
...
;;;;; tree-macs
     `(tree-macs-git-added-face ((,class (:foreground ,green :background ,green-bg))))
     `(tree-macs-git-conflict-face ((,class (:foreground ,red :background ,red-bg))))
     `(tree-macs-git-ignored-face ((,class (:foreground ,yellow))))
     `(tree-macs-git-modified-face ((,class (:foreground ,blue :background ,blue-bg))))
     `(tree-macs-git-untracked-face ((,class (:foreground ,aqua :background ,aqua-bg))))
...
(provide 'space-macs-common)

;; Local Variables:
;; no-byte-compile: t
;; End:

;;; space-macs-common.el ends here
