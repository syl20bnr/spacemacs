;;; core-funcs.el --- Space-macs Core File
;;
...
(defvar space-macs-repl-list '()
  "List of all registered REPLs.")

(defmacro space-macs|dotspace-macs-backward-compatibility (variable default)
  "Return `if' sexp for backward compatibility with old dotspace-macs
values."
  `(if (boundp ',variable) ,variable ',default))

(defun space-macs/system-is-mac ()
  (eq system-type 'darwin))
(defun space-macs/system-is-linux ()
  (eq system-type 'gnu/linux))
(defun space-macs/system-is-mswindows ()
  (eq system-type 'windows-nt))

(defun space-macs/window-system-is-mac ()
  ;; ns is returned instead of mac on e-macs 25+
  (memq (window-system) '(mac ns)))

(defun space-macs/mplist-get-values (plist prop)
...
(defun space-macs/mplist-get-value (plist prop)
...
Currently this function infloops when the list is circular."
  (car (space-macs/mplist-get-values plist prop)))

(defun space-macs/mplist-remove (plist prop)
...
(defun space-macs/dump-vars-to-file (varlist filename)
  "simplistic dumping of variables in VARLIST to a file FILENAME"
  (with-temp-file filename
    (space-macs/dump-vars varlist (current-buffer))
    (make-directory (file-name-directory filename) t)))

;; From http://stackoverflow.com/questions/2321904/elisp-how-to-save-data-in-a-file
(defun space-macs/dump-vars (varlist buffer)
  "insert into buffer the setq statement to recreate the variables in VARLIST"
  (cl-loop for var in varlist do
           (print (list 'setq var (list 'quote (symbol-value var)))
                  buffer)))

(defvar space-macs--init-redisplay-count 0
  "The number of calls to `redisplay'")
(defun space-macs//redisplay ()
  "`redisplay' wrapper."
  (setq space-macs--init-redisplay-count (1+ space-macs--init-redisplay-count))
  (force-window-update)
  (redisplay))

(defun space-macs//create-key-binding-form (props func)
  "Helper which returns a from to bind FUNC to a key according to PROPS.

Supported properties:

`:evil-leader STRING'
    One or several key sequence strings to be set with `space-macs/set-leader-keys .
...
`:define-key CONS CELL'
  ...
  (let ((evil-leader (space-macs/mplist-get-values props :evil-leader))
        (evil-leader-for-mode (space-macs/mplist-get-values props :evil-leader-for-mode))
        (global-key (space-macs/mplist-get-values props :global-key))
        (def-key (space-macs/mplist-get-values props :define-key)))
    (append
     (when evil-leader
       `((dolist (key ',evil-leader)
           (space-macs/set-leader-keys key ',func))))
...
(defun space-macs/prettify-org-buffer ()
...
(defun space-macs/view-org-file (file &optional anchor-text expand-scope)
...
(defun space-macs//test-var (pred var test-desc)
...
passed-tests and total-tests."
  (let ((var-name (symbol-name var))
    ...
    (insert (format "** TEST: [[file:%s::%s][%s]] %s\n"
                    dotspace-macs-filepath var-name var-name test-desc))
...
(defun space-macs//test-list (pred varlist test-desc &optional element-desc)
  ...
  (let ((varlist-name (symbol-name varlist))
    ...
    (if element-desc
        (insert (format "** TEST: Each %s in [[file:%s::%s][%s]] %s\n"
                        element-desc dotspace-macs-filepath varlist-name
                        varlist-name test-desc))
      (insert (format "** TEST: Each element of [[file:%s::%s][%s]] %s\n"
                      dotspace-macs-filepath varlist-name varlist-name
                      test-desc)))
...
;; hide mode line
;; from http://bzg.fr/e-macs-hide-mode-line.html
...
;; https://github.com/syl20bnr/space-macs/issues/8414
(defun space-macs/recompile-elpa (arg &optional dir)
...
This is useful if you switch e-macs versions or there
...
If DIR is non-nil, use a given directory for recompilation instead of elpa."
  ...
  (let ((user-e-macs-dir (or dir user-e-macs-directory))
      ...
      (seq-do
       (lambda (fname)
         (when (file-exists-p fname)
           (delete-file fname)))
       (directory-files-recursively user-e-macs-directory "\\.elc$" t)))
    (byte-recompile-directory package-user-dir 0 arg)))

(defun space-macs/register-repl (feature repl-func &optional tag)
  "Register REPL-FUNC to the global list of REPLs SPACe-macs-REPL-LIST.
...
buffer."
  (push `(,(or tag (symbol-name repl-func))
          . (,feature . ,repl-func))
        space-macs-repl-list))

;; http://stackoverflow.com/questions/11847547/e-macs-regexp-count-occurrences
(defun space-macs/how-many-str (regexp str)
...
(defun space-macs/echo (msg &rest args)
  "Display MSG in echo-area without logging it in *Messages* buffer."
  (interactive)
  (let ((message-log-max nil))
    (apply 'message msg args)))

(defun space-macs/derived-mode-p (mode &rest modes)
...
(defun space-macs/alternate-buffer (&optional window)
  "Switch back and forth between current and last buffer in the
current window.

If `space-macs-layouts-restrict-spc-tab' is `t' then this only switches between
the current layouts buffers."
  (interactive)
  (cl-destructuring-bind (buf start pos)
      (if (bound-and-true-p space-macs-layouts-restrict-spc-tab)
...
(defun space-macs/alternate-window ()
...
(defun space-macs/comint-clear-buffer ()
  (interactive)
...
(defun space-macs/error-delegate ()
  "Decide which error API to delegate to.

Delegates to flycheck if it is enabled and the next-error buffer
is not visible. Otherwise delegates to regular e-macs next-error."
  (if (and (bound-and-true-p flycheck-mode)
           (not next-error-function))
      'flycheck
    'e-macs))

(defun space-macs/next-error (&optional n reset)
  "Dispatch to flycheck or standard e-macs error."
  (interactive "P")
  (let ((sys (space-macs/error-delegate)))
    (cond
     ((eq 'flycheck sys) (call-interactively 'flycheck-next-error))
     ((eq 'e-macs sys) (call-interactively 'next-error)))))

(defun space-macs/last-error ()
  "Go to last flycheck or standard e-macs error."
  (interactive)
  (when (save-excursion (space-macs/next-error))
    (evil-goto-line)
    (space-macs/previous-error)))

(defun space-macs/previous-error (&optional n reset)
  "Dispatch to flycheck or standard e-macs error."
  (interactive "P")
  (let ((sys (space-macs/error-delegate)))
    (cond
     ((eq 'flycheck sys) (call-interactively 'flycheck-previous-error))
     ((eq 'e-macs sys) (call-interactively 'previous-error)))))

(defvar-local space-macs--gne-min-line nil
  "The first line in the buffer that is a valid result.")
(defvar-local space-macs--gne-max-line nil
  "The last line in the buffer that is a valid result.")
(defvar-local space-macs--gne-cur-line 0
  "The current line in the buffer. (It is problematic to use
point for this.)")
(defvar-local space-macs--gne-line-func nil
  "The function to call to visit the result on a line.")

(defun space-macs/gne-next (num reset)
  "A generalized next-error function. This function can be used
as `next-error-function' in any buffer that conforms to the
Space-macs generalized next-error API.

The variables `space-macs--gne-min-line',
`space-macs--gne-max-line', and `space-macs--line-func' must be
set."
  (when reset (setq space-macs--gne-cur-line
                    space-macs--gne-min-line))
  (setq space-macs--gne-cur-line
        (min space-macs--gne-max-line
             (max space-macs--gne-min-line
                  (+ num space-macs--gne-cur-line))))
  (goto-char (point-min))
  (forward-line (1- space-macs--gne-cur-line))
  (funcall space-macs--gne-line-func
           (buffer-substring (point-at-bol) (point-at-eol))))

(defun space-macs/terminal-fix-mode-line-indicator-overlap (str)
  "Add a space between two mode line indicators,
to fix an overlapping issue, that occurs when
Space-macs is started in a terminal,
and a modes mode line name is diminished to:
- A unicode character followed by a non unicode character, ex: \" â’ºh\"
- Or to two unicode characters, ex: \" â’ºâ“—\""
