;;; core-themes-support.el --- Space-macs Core File
;;
...
(defconst e-macs-built-in-themes (cons 'default (custom-available-themes))
  "List of e-macs built-in themes")

(defvar space-macs--fallback-theme 'space-macs-dark
  "Fallback theme if user theme cannot be applied.")

(defvar space-macs--delayed-user-theme nil
  "Internal variable storing user theme to be installed.")

(defvar space-macs--cur-theme nil
...
(defface org-kbd
  ...
  "Face for displaying key bindings in Space-macs documents."
  :group 'org-faces)

(defconst space-macs-theme-name-to-package
  '(
    ...
    (base16-space-macs                 . base16-theme)
    ...
    (space-macs-dark                   . space-macs-theme)
    (space-macs-light                  . space-macs-theme)
...
(defvar space-macs-post-theme-change-hook nil
  "Hook run after theme has changed.")

(defun space-macs/get-theme-package-name (theme-name)
  "Returns the package theme for the given THEME name."
  (cond
   ;; built-in
   ((memq theme-name e-macs-built-in-themes) nil)
   ;; from explicit alist
   ((assq theme-name space-macs-theme-name-to-package)
    (cdr (assq theme-name space-macs-theme-name-to-package)))
   ;; fallback to <name>-theme
   (t (intern (format "%S-theme" theme-name)))))

(defun space-macs//get-theme-name (theme)
  "Return the name of THEME."
  (if (listp theme)
      (car theme)
    theme))

(defun space-macs//get-theme-package-directory (theme)
  "Return the THEME location on disk."
  (let* ((theme-name (space-macs//get-theme-name theme))
         (pkg-name (space-macs/get-theme-package-name theme-name))
...
(defun space-macs/load-default-theme (&optional fallback-theme disable)
  "Load default theme.
Default theme is the car of `dotspace-macs-themes'.
If FALLBACK-THEME is non-nil it must be a package name which will be loaded if
THEME cannot be applied."
  (space-macs/load-theme (car dotspace-macs-themes) fallback-theme disable))

(defun space-macs/load-theme (theme &optional fallback-theme disable)
  "Apply user theme.
If FALLBACK-THEME is non-nil it must be a package name which will be loaded if
THEME cannot be applied.
    ...
    (condition-case err
        (progn
          ...
          (unless (or (memq theme-name (custom-available-themes))
            ...
            (let ((pkg-dir (space-macs//get-theme-package-directory theme))
                  (pkg-name (space-macs/get-theme-package-name theme-name)))
              (when pkg-dir
          ...
          (unless (display-graphic-p)
            (eval `(space-macs|do-after-display-system-init
                    (load-theme ',theme-name t))))
          (setq-default space-macs--cur-theme theme-name))
      ('error
       (message "error: %s" err)
       (if fallback-theme
           ;; fallback to Space-macs default theme
           (progn
             (setq space-macs--delayed-user-theme theme-name)
             (space-macs/load-fallback-theme fallback-theme disable))
         ...
         (space-macs-buffer/warning
          (concat "An error occurred while applying "
                  ...
                  "Error was: %s")
          theme-name space-macs--fallback-theme err)
         (space-macs-buffer/warning
          (concat "Please check the value of \"dotspace-macs-themes\" in your "
                  "dotfile or open an issue \n"
                  "so we can add support for the theme \"%s\".")
          theme-name))))))

(defun space-macs/load-fallback-theme (theme &optional disable)
  "Apply the fallback theme.
If DISABLE is non-nil then disable all previously applied themes before applying
THEME."
  (let ((theme-name (space-macs//get-theme-name theme)))
    ;; pop up fallback theme to the top of the list
    (setq space-macs--cur-theme theme-name)
    (setq dotspace-macs-themes (delq theme-name dotspace-macs-themes))
    (add-to-list 'dotspace-macs-themes theme-name)
    (when disable
      (mapc 'disable-theme custom-enabled-themes))
    (load-theme theme-name t)
    (unless (display-graphic-p)
...
(defun space-macs/cycle-space-macs-theme (&optional backward)
  "Cycle through themes defined in `dotspace-macs-themes'.
When BACKWARD is non-nil, or with universal-argument, cycle backwards."
  (interactive "P")
  (let* ((theme-names (mapcar 'space-macs//get-theme-name dotspace-macs-themes))
         (themes (if backward (reverse theme-names) theme-names))
         (next-theme (car (or (cdr (memq space-macs--cur-theme themes))
                              ;; if current theme isn't in cycleable themes, start
                              ;; over
                              themes))))
    (when space-macs--cur-theme
      (disable-theme space-macs--cur-theme))
    (let ((progress-reporter
           (make-progress-reporter
            (format "Loading theme %s..." next-theme))))
      (space-macs/load-theme next-theme nil 'disable)
      (progress-reporter-done progress-reporter))))

(defun space-macs/cycle-space-macs-theme-backward ()
  "Cycle through themes defined in `dotspace-macs-themes' backward."
  (interactive)
  (space-macs/cycle-space-macs-theme t))

(defadvice load-theme (after space-macs/load-theme-adv activate)
  "Perform post load processing."
  (let ((theme (ad-get-arg 0)))
    ;; Without this a popup is raised every time e-macs25 starts up for
    ;; assignment to a free variable
    (with-no-warnings
      (setq space-macs--cur-theme theme))
    (space-macs/post-theme-init theme)))

(defun space-macs/theme-loader ()
  "Call appropriate theme loader based on completion framework."
  (interactive)
  (cond
   ((configuration-layer/layer-used-p 'helm)
    (call-interactively 'space-macs/helm-themes))
   ((configuration-layer/layer-used-p 'ivy)
    (call-interactively 'counsel-load-theme))
   (t (call-interactively 'load-theme))))

(defun space-macs/post-theme-init (theme)
  "Some processing that needs to be done when the current theme
has been changed to THEME."
  (interactive)
  (run-hooks 'space-macs-post-theme-change-hook))

(defun space-macs//add-theme-packages-to-additional-packages ()
  "Add all theme packages from `dotspace-macs-themes' to packages to install."
  (setq dotspace-macs--additional-theme-packages nil)
  (dolist (theme dotspace-macs-themes)
    (let* ((theme-name (space-macs//get-theme-name theme))
           (pkg-name (space-macs/get-theme-package-name theme-name))
      ...
      (when pkg-name
        ...
        (add-to-list 'dotspace-macs--additional-theme-packages theme2)))))
(add-hook 'configuration-layer-pre-load-hook
          'space-macs//add-theme-packages-to-additional-packages)
