;;; funcs.el --- Space-macs Completion Layer functions File for Space-macs
;;
...
(defun space-macs/helm-faces ()
  "Describe face."
  (interactive)
  (require 'helm-elisp)
  (let ((default (or (face-at-point) (thing-at-point 'symbol))))
    (helm :sources (helm-def-source--e-macs-faces
                    (format "%s" (or default "default")))
          :buffer "*helm faces*")))

(defun space-macs//hide-cursor-in-helm-buffer ()
  "Hide the cursor in helm buffers."
  (with-helm-buffer
    (setq cursor-in-non-selected-windows nil)))

(defun space-macs//set-dotted-directory ()
...
(defun space-macs//helm-make-source (f &rest args)
...
(defun space-macs//helm-find-files-enable-helm--in-fuzzy ()
...
(defun space-macs//helm-hide-minibuffer-maybe ()
...
(defun space-macs//helm-hjkl-navigation (style)
...
(defun space-macs//display-helm-window (buffer &optional resume)
  "Display the Helm window respecting `helm-position'."
  (let ((display-buffer-alist
         (list space-macs-helm-display-help-buffer-regexp
               ;; this or any specialized case of Helm buffer must be
               ;; added AFTER `space-macs-helm-display-buffer-regexp'.
               ;; Otherwise, `space-macs-helm-display-buffer-regexp' will
               ;; be used before
               ;; `space-macs-helm-display-help-buffer-regexp' and display
               ;; configuration for normal Helm buffer is applied for helm
               ;; help buffer, making the help buffer unable to be
               ;; displayed.
               space-macs-helm-display-buffer-regexp)))
    (helm-default-display-buffer buffer)))

(defun space-macs//unprevent-minibuffer-escape ()
  "Workaround for a helm-evil incompatibility.
See https://github.com/syl20bnr/space-macs/issues/3700"
  (when helm-prevent-escaping-from-minibuffer
    (define-key evil-motion-state-map
      [down-mouse-1] 'evil-mouse-drag-region)))

(defun space-macs//prevent-minibuffer-escape ()
  "Workaround for a helm-evil incompatibility.
See https://github.com/syl20bnr/space-macs/issues/3700"
  (when helm-prevent-escaping-from-minibuffer
...
(defun space-macs//define-helm-action-functions ()
  "Define Space-macs functions to pick actions."
  (dotimes (n 10)
    (let ((func (intern (format "space-macs/helm-action-%d" n)))
...
(defun space-macs/helm-ts-edit ()
  ...
  (cond
   ...
   ((string-equal "*helm find files*" helm-buffer)
    (space-macs/helm-find-files-edit))
   ((string-equal "*Helm Swoop*" helm-buffer)
    (helm-swoop-edit))))

(defun space-macs//helm-navigation-ts-on-enter ()
  "Initialization of helm transient-state."
  ;; faces
  (space-macs//helm-navigation-ts-set-face)
  (setq space-macs--helm-navigation-ts-face-cookie-minibuffer
        (face-remap-add-relative
         'minibuffer-prompt
         'space-macs-helm-navigation-ts-face)))

(defun space-macs//helm-navigation-ts-set-face ()
  "Set the face for helm header in helm navigation transient-state"
  (with-helm-window
    (setq space-macs--helm-navigation-ts-face-cookie-header
          (face-remap-add-relative
           'helm-header
           'space-macs-helm-navigation-ts-face))))

(defun space-macs//helm-navigation-ts-on-exit ()
  "Action to perform when exiting helm transient-state."
  (with-helm-window
    (face-remap-remove-relative
     space-macs--helm-navigation-ts-face-cookie-header))
  (face-remap-remove-relative
   space-macs--helm-navigation-ts-face-cookie-minibuffer))

(defun space-macs/helm-transient-state-select-action ()
...
(defun space-macs//ivy-hjkl-navigation (style)
  "Set navigation on 'hjkl' for the given editing STYLE."
  (cond
   ((or (eq 'vim style)
        (and (eq 'hybrid style)
...
(defun space-macs//ivy-matcher-desc ()
  (replace-regexp-in-string "ivy--" "" (format "%s" ivy--regex-function)))


;; Ido

(defun space-macs//ido-minibuffer-setup ()
  ...
  (when space-macs--ido-navigation-ts-enabled
    (space-macs/ido-navigation-transient-state/body)))

(defun space-macs//ido-setup ()
  (when space-macs--ido-navigation-ts-face-cookie-minibuffer
    (face-remap-remove-relative
     space-macs--ido-navigation-ts-face-cookie-minibuffer))
  ;; be sure to wipe any previous transient-state flag
  (setq space-macs--ido-navigation-ts-enabled nil)
  ...
  (define-key ido-completion-map (kbd "C-o") 'space-macs/ido-invoke-in-other-window)
  (define-key ido-completion-map (kbd "C-s") 'space-macs/ido-invoke-in-vertical-split)
  (define-key ido-completion-map (kbd "C-t") 'space-macs/ido-invoke-in-new-frame)
  (define-key ido-completion-map (kbd "C-v") 'space-macs/ido-invoke-in-horizontal-split)
  ;; initiate transient-state
  (define-key ido-completion-map (kbd "M-SPC") 'space-macs/ido-navigation-transient-state/body)
  (define-key ido-completion-map (kbd "S-M-SPC") 'space-macs/ido-navigation-transient-state/body))

(defun space-macs/ido-invoke-in-other-window ()
  "signals ido mode to switch to (or create) another window after exiting"
  (interactive)
  (setq ido-exit-minibuffer-target-window 'other)
  (ido-exit-minibuffer))

(defun space-macs/ido-invoke-in-horizontal-split ()
  "signals ido mode to split horizontally and switch after exiting"
  (interactive)
  (setq ido-exit-minibuffer-target-window 'horizontal)
  (ido-exit-minibuffer))

(defun space-macs/ido-invoke-in-vertical-split ()
  "signals ido mode to split vertically and switch after exiting"
  (interactive)
  (setq ido-exit-minibuffer-target-window 'vertical)
  (ido-exit-minibuffer))

(defun space-macs/ido-invoke-in-new-frame ()
  "signals ido mode to create a new frame after exiting"
  (interactive)
  (setq ido-exit-minibuffer-target-window 'frame)
  (ido-exit-minibuffer))

(defun space-macs//ido-navigation-ts-set-face ()
  "Set faces for ido navigation transient-state."
  (setq space-macs--ido-navigation-ts-face-cookie-minibuffer
        (face-remap-add-relative
         'minibuffer-prompt
         'space-macs-ido-navigation-ts-face)))

(defun space-macs//ido-navigation-ts-on-enter ()
  "Initialization of ido transient-state."
  (setq space-macs--ido-navigation-ts-enabled t)
  (space-macs//ido-navigation-ts-set-face))

(defun space-macs//ido-navigation-ts-on-exit ()
  "Action to perform when exiting ido transient-state."
  (face-remap-remove-relative
   space-macs--ido-navigation-ts-face-cookie-minibuffer))

(defun space-macs//ido-navigation-ts-full-doc ()
