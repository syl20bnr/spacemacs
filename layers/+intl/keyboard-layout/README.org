...
add =keyboard-layout= to the existing =dotspace-macs-configuration-layers= list
...
=kl/pre-config-<NAME>= and =kl/post-config-<NAME>= in =dotspace-macs/user-init=.
...
that were introduced by these changes in other parts of Space-macs. This layer
...
~META+KEY~ and leader key bindings such as (~SPC â€¦~), whenever it makes sense.
...
shortcuts:
- Map the unused ~Ã©~ key as an alias for ~w~, it's more useful in vim mode:
  - ~Ã© â†’ w~
  - ~Ã‰ â†’ W~
...
~C-w~. This avoids having to change its meaning in other modes. Space-macs
...
This Space-macs Keyboard Layout layer has two Dvorak movement key variants:
...
*Note*: for technical reasons (see [[https://github.com/syl20bnr/space-macs/pull/7178#issuecomment-249360301][GH-7178]]), when using =colemak-jkhl=, the
