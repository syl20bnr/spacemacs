...
* Keyboard layouts
The following keyboard-layouts are available with this layer, and you can easily add your own configurations by editing the config.el file of this layer (you just need to set which keys to swap and the rest Just Works):
...
file:img/dvorak-right-handed-layout.png
...
This Spacemacs Keyboard Layout layer has 3 Dvorak movement key variants:
...
The =dvorak-right-handed= variant: matches Vims movement keys:
- ~h → h~
- ~t → j~
- ~d → k~
- ~c → l~
