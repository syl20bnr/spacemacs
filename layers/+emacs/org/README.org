...
To install [[https://github.com/nobiot/org-transclusion][org-transclusion]], which includes a read-only copy of textual content
via a file or ID link toggles, set the =org-enable-transclusion-support= to =t=:
