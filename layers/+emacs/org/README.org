...
the [[https://github.com/syl20bnr/space-macs/blob/develop/layers/+lang/bibtex/README.org][BibTeX layer]].
...
of the =org= shipped with e-macs which will induce conflicts.
...
layer instead of manually loading =org= as separate e-macs package.

If this is not done you will encounter a lot of unbind key exceptions while working with org.
More details can be found [[https://github.com/syl20bnr/space-macs/issues/8106][here]].

* Install
** Layer
To use this configuration layer: in the main Space-macs configuration
file (=~/.space-macs=), to the existing =dotspace-macs-configuration-layers= list
add the =org= entry.
...
the =gnuplot= package to your =dotspace-macs-excluded-packages= variable.
...
This would install the [[https://gitlab.com/oer/org-re-reveal/][org-re-reveal]] e-macs extension.
...
- Easily integrates with org-agenda and e-macs calendar, iCalendar,
  allows scheduling
...
this - set =org-journal-dir= variable in the =dotspace-macs/user-config=:
...
- define the following in =dotspace-macs/user-config=:
...
Any of the org-journal settings can be configured in =dotspace-macs/user-config=
...
=dotspace-macs/user-config= of your dotfile by setting the variable
=org-superstar-headline-bullets-list=. By default the list is set to =("â—‰" "â—‹" "âœ¸" "âœ¿")=.

#+BEGIN_SRC e-macs-lisp
  (setq org-superstar-bullet-list '("â– " "â—†" "â–²" "â–¶"))
#+END_SRC

You can disable the fancy bullets entirely by adding =org-superstar= to =dotspace-macs-excluded-packages=.

#+BEGIN_SRC e-macs-lisp
  (dotspace-macs-excluded-packages '(org-superstar))
#+END_SRC
...
** Org-brain support
For e-macs 25 or later, support for [[https://kungsgeten.github.io/org-brain.html][org-brain]] is included. See the [[https://github.com/Kungsgeten/org-brain][org-brain
...
| Key binding                                  | Description                                   |
|----------------------------------------------+-----------------------------------------------|
| ~SPC m <dotspace-macs-major-mode-leader-key>~ | org-ctrl-c-ctrl-c                             |
...
| ~SPC s j~                                    | space-macs/jump-in-buffer (jump to a heading)  |
...
| ~SPC m i K~   | space-macs/insert-keybinding-org               |
...
| Key binding                                  | Description                            |
|----------------------------------------------+----------------------------------------|
| ~SPC m <dotspace-macs-major-mode-leader-key>~ | confirm in =org-capture-mode=          |
