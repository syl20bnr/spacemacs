;;; hybrid-mode.el --- Put one foot in the church of e-macs
...
(defadvice evil-evilified-state (around hybrid-evilified-to-hybrid-state disable)
  ...
  (if (equal -1 (ad-get-arg 0))
    ...
    (if hybrid-style-enable-evilified-state
        ad-do-it
      ;; seems better to set the e-macs state instead of hybrid for evilified
      ;; buffers
      (evil-e-macs-state))))
...
(defun enable-hybrid-editing-style ()
  ...
  (run-hook-with-args 'space-macs-editing-style-hook 'hybrid)
...
;; see https://github.com/e-macs-evil/evil/blob/56e92f7cb4e04e665670460093b41f58446b7a2b/evil-states.el#L108
