;;; funcs.el --- rcirc Layer functions File for Space-macs
;;
...
(defun space-macs/rcirc (arg)
  "Launch rcirc."
  (interactive "P")
  (require 'rcirc)
  ;; dispatch to rcirc launcher with appropriate support
  (cond
   (rcirc-enable-authinfo-support (space-macs//rcirc-with-authinfo arg))
   (rcirc-enable-znc-support (space-macs//rcirc-with-znc arg))
   (t (rcirc arg))))

(defun space-macs//rcirc-with-authinfo (arg)
  "Fire rcirc with support for authinfo."
  (unless arg
    (if (file-exists-p "~/.authinfo.gpg")
        (space-macs//rcirc-authinfo-config)
      (message "Cannot find file ~/.authinfo.gpg")))
  (rcirc arg))

(defun space-macs//rcirc-with-znc (arg)
  ...
  (if arg
    ...
    (setq rcirc-server-alist
          ...
          (space-macs//znc-rcirc-server-alist-get-authinfo
           rcirc-server-alist))
    (space-macs//znc-rcirc-connect)))

(defun space-macs/rcirc-notify-beep (msg)
  "Beep when notifying."
  (let ((player "mplayer")
        (sound (concat space-macs-start-directory "site-misc/startup.ogg")))
...
(defun space-macs//rcirc-persp-filter-save-buffers-function (buffer)
  "Filter for rcirc layout."
  (with-current-buffer buffer
    (eq major-mode 'rcirc-mode)))

(defun space-macs//rcirc-buffer-to-persp ()
  "Add buffer to rcirc layout."
  (persp-add-buffer (current-buffer) (persp-get-by-name
                                      rcirc-space-macs-layout-name)))


;; logging

(defun space-macs//rcirc-write-log (process sender response target text)
...
(defun space-macs/rcirc-insert-current-emms-track ()
  (interactive)
...
(defun space-macs//rcirc-authinfo-config ()
  "Initialize authinfo.
...
(defun space-macs//znc-auth-source-fetch-password (server)
...
(defun space-macs//znc-rcirc-server-alist-get-authinfo (server-alist)
 ...
 from .authinfo.gpg"
  (dolist (server server-alist server-alist)
    (let* ((host  (car server))
           ...
           (pass  (space-macs//znc-auth-source-fetch-password server)))
      (when auth
...
(defun space-macs//znc-rcirc-connect ()
...
(defun space-macs//rcirc-image-show-url (_sender _response)
...
(defun space-macs//rcirc-youtube-show-info (_sender _response)
  (erc-yt-show-info))

;; erc-tweet --------------------------------------------------------------------

(defun space-macs//rcirc-tweet-show-tweet (_sender _response)
