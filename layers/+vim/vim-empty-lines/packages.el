...
(defun vim-empty-lines/init-vim-empty-lines-mode ()
  (use-package vim-empty-lines-mode
    :init
    (space-macs/add-to-hooks (lambda () (vim-empty-lines-mode -1))
                            '(comint-mode-hook
                              eshell-mode-hook
                              eww-mode-hook
                              shell-mode-hook
    ...
    :config
    (progn
      (space-macs|hide-lighter vim-empty-lines-mode)
      (global-vim-empty-lines-mode)
      (space-macs|add-toggle vim-empty-lines-mode
        :mode global-vim-empty-lines-mode
        :documentation
        "Display an overlay of ~ on empty lines."
        :evil-leader "t~")
      ;; Don't enable it where it is detrimental.
      (dolist (x (list space-macs-buffer-name
                       "*Messages*"))
