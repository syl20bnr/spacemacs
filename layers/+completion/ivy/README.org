...
- =ivy-enable-posframe= Enable =ivy-posframe=. See [[https://github.com/tumashu/ivy-posframe][here]].
...
** ivy-posframe
 To enable =ivy-posframe=, set the variable =ivy-enable-posframe= to =t=.

#+BEGIN_SRC emacs-lisp
  (setq-default dotspacemacs-configuration-layers '(
    (ivy :variables ivy-enable-posframe t)))
#+END_SRC
