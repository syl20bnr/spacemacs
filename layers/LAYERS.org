...
[[file:+checkers/harper/README.org][+checkers/harper/README.org]]

Features:
- Offline grammar and spell checking via Harper's Rust based language server.
- Integrated LSP diagnostics through Eglot, showing errors, warnings, and suggestions.
- Quick fix actions and code actions accessible via `,` and `'` keys.
- Toggle Harper for the current buffer with `SPC S h`.
- Customizable binary path and arguments via `harper-ls-bin` and `harper-ls-args` layer variables.
