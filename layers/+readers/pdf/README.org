...
On macOS, the default instructions might not work. There is a brew tap at dunn/emacs that contains pdf-tools, but it did not work for me. I used the following successfully. Note that your PATH should be set correctly in Emacs, and that pdf-tools does not work in terminal (GUI-only).
...
~B~ in Emacs state.)
On some machines, pdf-tools can be quite slow (especially with a line-number mode active). You might want to try alternatives such as zathura.
