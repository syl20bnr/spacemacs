...
add =colors= to the existing =dotspace-macs-configuration-layers= list in this
file.
...
Instead, you must push themes into the alist in your =dotspace-macs/user-config= section,
...
It may be handy to enable it only when e-macs is running in a GUI.
You can do that by using a quasi-quoted list which checks the e-macs mode:
...
GUI e-macs and that you haven't disabled sRGB mode (if your platform uses that).
