  ...
  - [[#backends][Backends]]
    - [[#lsp][LSP]]
      ...
      - [[#configuration][Configuration]]
        ...
        - [[#example-dotspace-macs-configuration-layers-entry][Example dotspace-macs-configuration-layers entry]]
...
add =c-c++= to the existing =dotspace-macs-configuration-layers= list in this
file.
...
- =ycmd= using [[https://github.com/abingham/e-macs-ycmd][e-macs-ycmd]]
...
e-macs =lsp-mode= package and probably the easiest to install.
...
relies on e-macs' fastest =font-lock= mechanism. A more
...
- [[https://github.com/MaskRay/ccls/wiki/e-macs][e-macs section of =ccls= wiki]]

***** Example dotspace-macs-configuration-layers entry
#+BEGIN_SRC e-macs-lisp
  (setq-default dotspace-macs-configuration-layers
                '((c-c++ :variables
...
The mode for header files is auto detected by `c-or-c++-mode' in e-macs > 26.1+.
Older versions of e-macs will open header files in =c-mode= by default, you can open them in
...
can be found in the manual: [[http://www.gnu.org/software/e-macs/manual/html_node/elisp/Directory-Local-Variables.html][dir-locals]].
...
[[https://www.e-macswiki.org/e-macs/TrampMode][Tramp]] remote editing on systems that don't have =clang-format= installed, you
...
C-C++ lang in Space-macs. In your =~/.space-macs= file, a possible way that this
would look is that in your list of =dostpace-macs-configuration-layers= you have
an entry like
