...
* Backends
There are currently two language backends supported for the scala language in Spacemacs: ENSIME and METALS. When declarating the scala layer, you can set the  preferred backend to load. The current default is scala-ensime
