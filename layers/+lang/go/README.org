...
** YCMD backend
To enable the YCMD backend set the layer variable =go-backend=:

#+BEGIN_SRC elisp
(setq go-backend 'ycmd)
#+END_SRC

or

#+BEGIN_SRC elisp
(go :variables go-backend 'ycmd)
#+END_SRC

You also need to install [[https://github.com/Valloric/ycmd][YCMD]] with =go-completer= and add =ycmd= to =dotspacemacs-configuration-layers=.
For more information read [[https://github.com/syl20bnr/spacemacs/tree/develop/layers/%2Btools/ycmd][YCMD layer README.org]]
and [[https://github.com/abingham/emacs-ycmd#company-ycmd][emacs-ycmd README.md]]
