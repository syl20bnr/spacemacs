...
(defconst emacs-lisp-packages
  '(auto-compile
    ...
    (inspector
     :location (recipe :fetcher github :repo "mmontone/emacs-inspector"))
    macrostep
    nameless
    overseer
    rainbow-identifiers
    semantic
    smartparens
    (elisp-autofmt :toggle emacs-lisp-format-on-save)
    srefactor
    emr))


(defun emacs-lisp/init-elisp-autofmt ()
  (use-package
   elisp-autofmt
   :defer t
   ...
   (elisp-autofmt-cache-directory
    (let ((dir
           (expand-file-name "elisp-autofmt-cache/" spacemacs-cache-directory)))
      (make-directory dir t)
      dir))
   :init (add-hook 'emacs-lisp-mode-hook 'elisp-autofmt-mode)))

(defun emacs-lisp/init-ielm ()
  (use-package
   ielm
   :defer t
   :init (spacemacs/register-repl 'ielm 'ielm)
   ;; Load better help mode if helpful is installed
   (if (configuration-layer/layer-used-p 'helpful)
       (spacemacs/set-leader-keys-for-major-mode
        'inferior-emacs-lisp-mode "hh" 'helpful-at-point)
     (spacemacs/set-leader-keys-for-major-mode
      'inferior-emacs-lisp-mode
      "hh"
      'elisp-slime-nav-describe-elisp-thing-at-point))
   (add-to-list
    'spacemacs-jump-handlers-inferior-emacs-lisp-mode
    'elisp-slime-nav-find-elisp-thing-at-point)
   (dolist (mode '(emacs-lisp-mode lisp-interaction-mode))
     (spacemacs/declare-prefix-for-mode mode "ms" "ielm")
     (spacemacs/set-leader-keys-for-major-mode mode "'" 'ielm "si" 'ielm))
   :config
   (defun ielm-indent-line ()
     ...
     (let ((current-point (point)))
       (save-restriction
         (narrow-to-region
          (search-backward-regexp "^ELISP>") (goto-char current-point))
         (lisp-indent-line))))))
...
(defun emacs-lisp/init-debug ()
  (use-package
   ...
   (dolist (mode '(emacs-lisp-mode lisp-interaction-mode))
     ...
     (spacemacs/set-leader-keys-for-major-mode
      mode "dt" 'spacemacs/elisp-toggle-debug-expr-and-eval-func))
   :config (evilified-state-evilify-map debugger-mode-map :mode debugger-mode)))
  ...
  (use-package
   ...
   (dolist (mode '(emacs-lisp-mode lisp-interaction-mode))
     (spacemacs/set-leader-keys-for-major-mode
      mode
      "df"
      'spacemacs/edebug-instrument-defun-on
      "dF"
      'spacemacs/edebug-instrument-defun-off))
   ...
   (evil-set-custom-state-maps
    ...
    (delq (assq 'edebug-mode-map evil-intercept-maps) evil-intercept-maps))
   (evilified-state-evilify-map
    edebug-mode-map
    :eval-after-load edebug
    :pre-bindings ; Remove key bindings that cannot be evilified
    "G"
    nil ; edebug-Go-nonstop-mode
    :bindings
    "a"
    'edebug-stop
    "c"
...
(defun emacs-lisp/init-auto-compile ()
  (use-package
   ...
   :config (spacemacs|hide-lighter auto-compile-mode)
   (spacemacs/set-leader-keys-for-major-mode
    'emacs-lisp-mode "cl" 'auto-compile-display-log)))
...
(defun emacs-lisp/init-elisp-slime-nav ()
  ...
  (use-package
   ...
   (dolist (mode '(emacs-lisp-mode lisp-interaction-mode))
     ...
     ;; Load better help mode if helpful is installed
     (if (configuration-layer/layer-used-p 'helpful)
         (spacemacs/set-leader-keys-for-major-mode mode "hh" 'helpful-at-point)
       (spacemacs/set-leader-keys-for-major-mode
...
(defun emacs-lisp/init-emacs-lisp ()
  ...
  (dolist (mode '(emacs-lisp-mode lisp-interaction-mode))
    ...
    (spacemacs/set-leader-keys-for-major-mode
     ...
     'xref-go-back
     "gG"
     'spacemacs/nav-find-elisp-thing-at-point-other-window
     ","
...
(defun emacs-lisp/init-macrostep ()
  (use-package
   macrostep
   :defer t
   :init (evil-define-key 'normal macrostep-keymap "q" 'macrostep-collapse-all)
   ...
   (spacemacs/set-leader-keys-for-major-mode
    'emacs-lisp-mode "dm" 'spacemacs/macrostep-transient-state/body)))
...
(defun emacs-lisp/post-init-evil ()
  (add-hook
   'emacs-lisp-mode-hook #'spacemacs//define-elisp-comment-text-object))
...
(defun emacs-lisp/init-flycheck-package ()
  (use-package
   ...
   :defer t
   :init
   (spacemacs|add-transient-hook
    emacs-lisp-mode-hook emacs-lisp//flycheck-package-setup)))
...
(defun emacs-lisp/init-flycheck-elsa ()
  (use-package
   ...
   :init
   (spacemacs|add-transient-hook
    emacs-lisp-mode-hook emacs-lisp//flycheck-elsa-setup)))
...
(defun emacs-lisp/post-init-rainbow-identifiers ()
  (add-hook
   'emacs-lisp-mode-hook #'colors//rainbow-identifiers-ignore-keywords))
...
(defun emacs-lisp/post-init-smartparens ()
  (dolist (mode '(emacs-lisp-mode lisp-interaction-mode))
    (spacemacs/set-leader-keys-for-major-mode
     ...
     "es"
     'spacemacs/eval-current-symbol-sp)))
...
(defun emacs-lisp/init-emr ()
  (use-package
   ...
   (let ((key-binding-prefixes
     ...
     (mapc
      (lambda (x)
        (spacemacs/declare-prefix-for-mode 'emacs-lisp-mode (car x) (cdr x)))
...
(defun emacs-lisp/init-inspector ()
  (use-package
   ...
   :defer t
   :config
   (evilified-state-evilify-map inspector-mode-map :mode inspector-mode)))
