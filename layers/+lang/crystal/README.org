...
add =crystal= to the existing =dotspace-macs-configuration-layers= list in this
file.
...
by e-macs, once a crystal file is opened. The key bindings are
...
** REPL
Start a inferior crystal REPL process with ~SPC m 'â€‹~ or ~SPC m s i~.
Then you can send code to it with the bindings below:

| Key binding | Description                                 |
|-------------+---------------------------------------------|
| ~SPC m 'â€‹~   | start inferior REPL process                 |
