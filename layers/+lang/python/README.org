...
| ~SPC m g g~ | go to definition using =anaconda-mode-goto-definitions= (~C-o~ to jump back) |
| ~SPC m g a~ | go to assignment using =anaconda-mode-goto-assignments= (~C-o~ to jump back) |
| ~SPC m g u~ | navigate between usages with =anaconda-mode-goto-references=                 |
