...
the pyenv version or if no such file is found set the global pyenv version. The
behavior can be set with the variable =python-auto-set-local-pyenv-version= to:
