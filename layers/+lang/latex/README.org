...
|--------------------------+--------------------------------------------------------------------+--------------------------------------------------------------------------------------------|
| ~latex-backend~          | ~nil~                                                              | Use LSP backend, unless it's `company-auctex`, `company-math` or =LSP= layer isn't enabled |
...
*** Company-math 
This mode is using =company-auctex= together the =company-math= package. The package
=company-math= has a wider coverage of =LaTeX= mathematical symbols. Moreover, it
displays a unicode preview of the symbols in the completion menu.
...
#+BEGIN_SRC emacs-lisp
  (setq-default dotspacemacs-configuration-layers
                '((latex :variables latex-backend 'company-math)))
#+END_SRC
