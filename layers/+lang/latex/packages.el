;;; packages.el --- Latex Layer packages File for Space-macs
;;
...
(defun latex/post-init-company ()
  (space-macs//latex-setup-company))
...
(defun latex/init-auctex ()
  (use-package tex
    ...
    (progn
      ...
      (add-hook 'LaTeX-mode-hook #'space-macs//latex-setup-backend))
    :config
    (progn
      ...
      (dolist (mode '(tex-mode latex-mode context-mode))
        ...
        (space-macs/declare-prefix-for-mode mode "mxf" "fonts")
        (unless (and (eq (space-macs//latex-backend) 'lsp)
                     (eq mode 'latex-mode))
          (space-macs/declare-prefix-for-mode mode "mh" "help")
          (space-macs/declare-prefix-for-mode mode "mx" "text/fonts")
          (space-macs/set-leader-keys-for-major-mode mode
            "a"   'TeX-command-run-all                         ;; C-c C-a
            "b"   'latex/build))
        (when dotspace-macs-major-mode-e-macs-leader-key
          (space-macs/set-leader-keys-for-major-mode mode
            dotspace-macs-major-mode-e-macs-leader-key 'TeX-command-master))
        (when dotspace-macs-major-mode-leader-key
          (space-macs/set-leader-keys-for-major-mode mode
            dotspace-macs-major-mode-leader-key 'TeX-command-master))
        (when latex-enable-folding
          (space-macs/set-leader-keys-for-major-mode mode
            ...
            "zz" 'TeX-fold-dwim)
          (space-macs/declare-prefix-for-mode mode "mz" "fold")))
      ...
      (if (eq (space-macs//latex-backend) 'lsp)
          (space-macs/set-leader-keys-for-major-mode 'latex-mode
      ...
      ;; Declare prefixes
      (space-macs/declare-prefix-for-mode 'latex-mode "mi" "insert")
      (space-macs/declare-prefix-for-mode 'latex-mode "mp" "preview")
      (space-macs/declare-prefix-for-mode 'latex-mode "mf" "fill"))))

(defun latex/pre-init-auctex-latexmk ()
  (space-macs|use-package-add-hook tex
    :post-config
...
(defun latex/post-init-flycheck ()
  (space-macs/enable-flycheck 'latex-mode))
...
(defun latex/init-reftex ()
  ...
  (setq reftex-plug-into-AUCTeX '(nil nil t t t)
        reftex-use-fonts t)
  (let ((prefix (if (eq (space-macs//latex-backend) 'lsp) "R" "r")))
    (space-macs/declare-prefix-for-mode 'latex-mode (concat "m" prefix) "reftex")
    (space-macs/set-leader-keys-for-major-mode 'latex-mode
...
(defun latex/post-init-counsel-gtags ()
  (space-macs/counsel-gtags-define-keys-for-mode 'latex-mode))

(defun latex/post-init-helm-gtags ()
  (space-macs/helm-gtags-define-keys-for-mode 'latex-mode))

(defun latex/post-init-ggtags ()
  (add-hook 'latex-mode-local-vars-hook #'space-macs/ggtags-mode-enable))
...
(defun latex/post-init-typo ()
  ...
  (defun space-macs//disable-typo-mode ()
    (typo-mode -1))
  (add-hook 'LaTeX-mode-hook 'space-macs//disable-typo-mode))

(defun latex/post-init-yasnippet ()
  (add-hook 'LaTeX-mode-hook 'space-macs/load-yasnippet))
