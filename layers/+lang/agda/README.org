...
add =agda= to the existing =dotspace-macs-configuration-layers= list in this
file.
...
Then check that =agda= is available on your =$PATH= and seen by e-macs. For
...
to your needs. Set it to =nil= if =agda2.el= is already discoverable in e-macs'
...
* Key bindings
The key bindings of this layer don't follow the Space-macs conventions,
we opted to a simple transcription of stock Agda mode key bindings to
Space-macs leader key.
...
| ~SPC m â€‹,â€‹~   | Shows the type of the goal at point and the currect context.                                                                              |
