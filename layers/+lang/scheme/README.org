...
add =scheme= to the existing =dotspace-macs-configuration-layers= list in this
file.
...
to your =dotspace-macs/user-config= in order for the REPL to start in space-macs.
...
#+BEGIN_SRC e-macs-lisp
  (space-macs/toggle-evil-safe-lisp-structural-editing-on-register-hook-scheme-mode)
#+END_SRC
