...
You can check what is wrong with flychec with the =flycheck-compile= command. 
...
You can force the checker by using =flycheck-set-checker= (~SPC e s~) to =haskell-stack-ghc=.
