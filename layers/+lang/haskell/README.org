...
- [[#faq][FAQ]]
  ...
  - [[#why-doesnt-the-hlint-flycheck-checker-work-when-intero-the-backend-is-enabled][Why doesn't the HLint Flycheck checker work when Intero the backend is enabled?]]
...
** Why doesn't the HLint Flycheck checker work when Intero the backend is enabled? 
...
To also enable the HLint checker use =flycheck-add-next-checker= to set the HLint checker after the Intero checker.
