...
| ~SPC a t s g~ | Toggle pop-shell with =ghostel=                          |
|---------------+----------------------------------------------------------|
...
** ghostel
Ghostel is a terminal emulator powered by `libghostty`. It offers fast rendering
and built‑in copy/Emacs navigation modes. The layer provides:
- REPL registration (`spacemacs/register-repl 'ghostel 'ghostel`).
- A `shell-pop` command (`make-shell-pop-command "ghostel" ghostel`).
- Leader key `SPC a t s g` (bound to `spacemacs/shell-pop-ghostel`).
- Toggle variable `shell-enable-ghostel-support` (default `t`).
- Requires the system package `libghostty`.
