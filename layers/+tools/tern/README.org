...
add =tern= to the existing =dotspace-macs-configuration-layers= list in this
file.
...
* Configuration
** Custom tern command
Windows users may need to set the variable =tern-command= in order for e-macs to
locate and launch tern server successfully. See [[https://github.com/syl20bnr/space-macs/issues/5733][this open issue for more
